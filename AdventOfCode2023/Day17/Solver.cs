namespace AdventOfCode2023.Day17
{
    ...
    public class Solver : ISolver
    {
        ...
        private static int CalcRoute(WeightedGrid grid, int minLeg, int maxLeg)
        {
            var astar = new AStar(
                graph: grid,
                start: "0,0",
                finish: $"{grid.Width-1},{grid.Height-1}",
                minStraightLine: minLeg,
                maxStraightLine: maxLeg
                );

            Console.WriteLine(grid.Draw(astar));

            return astar.TotalCost;
        }
